...
public class MovieAnalyze {
	...
	public static void main(String[] args) {
		...
		String[] genre = {"어드벤처","액션","판타지","경이","모험","로맨스","스릴러","공포","다큐","드라마","추리","애니메이션","코미디","뮤지컬"};
		...
		for(int i=0;i<reply.length;i++) {
			String regex = "[A-Za-z0-9<>~ㅋㅎ().?:/]"; // 이런것들 제거시킬 예정
			reply[i] = reply[i].replaceAll(regex, ""); // String클래스의 replaceAll 사용
			/*System.out.println(reply[i]);*/
			
			for(int j=0;j<genre.length;j++) {
				
				int count = 0;
				Pattern p = Pattern.compile(genre[j]);
